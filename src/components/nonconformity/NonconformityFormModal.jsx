...
import {
  ...
  User, Building2, Calendar, Car, Layers, FileText, Wrench, MessageSquare, Loader2
} from 'lucide-react';
...
const NonconformityFormModal = ({ isOpen, setIsOpen, record, onSaveSuccess }) => {
  ...
  const [personnelFetchLoading, setPersonnelFetchLoading] = useState(false);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (personnelRes.error) {
          console.warn('Personel listesi alınamadı:', personnelRes.error);
          setPersonnel([]);
        } else {
          setPersonnel(personnelRes.data || []);
        }
        if (deptRes.error) {
          console.warn('Birim listesi alınamadı:', deptRes.error);
          setDepartments([]);
        } else {
          setDepartments(deptRes.data || []);
        }
      } finally {
        setPersonnelFetchLoading(false);
      }
    };
    fetchData();
  }, [isOpen]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 p-4">
            ...
            <div>
              ...
              {personnelOptions.length > 0 ? (
                ...
                <Input id="detected_by" value={formData.detected_by} onChange={handleChange} placeholder="Ad Soyad" />
              )}
            </div>
            <div>
