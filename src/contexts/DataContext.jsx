...
export const DataProvider = ({ children }) => {
    ...
    const fetchData = useCallback(async (forceRefresh = false) => {
        ...
        const instantPromises = {
            personnel: supabase.from('personnel').select('id, full_name, email, avatar_url, department, unit_id, is_active, registration_number, job_title, management_department, collar_type').order('full_name'),
        ...
        try {
            ...
            instantResults.forEach((result, index) => {
                ...
                if (val?.error) {
                    if (key === 'personnel') {
                        console.warn('⚠️ Personel (DataContext) çekilemedi:', val.error.message || val.error);
                    }
                    newState[key] = [];
                    return;
                }
